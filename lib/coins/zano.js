...
function Coin(data){
    ...
    this.constructNewBlob = function(blockTemplate, NonceBuffer){
        return zanoUtil.construct_block_blob(blockTemplate, NonceBuffer);
    };
    ...
    this.getPoWHash = function(convertedBlob, nonce, height) {
	   console.log('getPowHash: block_header_hash: ' + convertedBlob.toString('hex') + '; nonce: ' + nonce.toString('hex') + '; nonce dec: ' + nonce.toString() + '; height: ' + height.toString('hex') + ';height dec = ' + height.toString());
let         hash = zanoUtil.get_pow_hash(convertedBlob, nonce, height);
	    return hash;
    };

    this.getHashFromBlockTemplateWithExtra = function(blockTemplate, extraData) {
	    console.log('this.getHashFromBlockTemplateWithExtra: blockTemplate: ' + blockTemplate.toString('hex') + '; extraData: ' + extraData.toString('hex'));
       let result = zanoUtil.get_hash_from_block_template_with_extra(blockTemplate, extraData);
	    console.log('this.getHashFromBlockTemplateWithExtra result: ' + result.toString('hex'));
	    return result;
    };
