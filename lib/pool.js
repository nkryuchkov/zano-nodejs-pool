    ...
    if (protoVersion === 1) {
        ...
        this.getJob = function () {
            ...
            if (!this.proxy) {
                ...
                let newJob = [
                    crypto.pseudoRandomBytes(21).toString('base64'),
                    '' + activeBlockTemplate.extraNonce,
                    activeBlockTemplate.idHash,
                ];

                this.validJobs.enq(newJob);
                this.cachedJob = [newJob[0], newJob[1], newJob[2]];
            } else {
...
function processShare(miner, job, blockTemplate, params, sendReply) {
    ...
    if (global.config.pool.trustedMiners && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 &&
        ...
        hash = global.coinFuncs.hash(convertedBlob);
        shareType = false;
    }
