...
//	    console.log('rpc response: ' + JSON.stringify(rpcResponse));
...
function Miner(id, login, pass, ipAddress, startingDiff, messageSender, protoVersion, portType, port, agent) {
    ...
    if (protoVersion === 1) {
		...
		console.log('getting target hex');
            if (this.newDiff) {
		    console.log('newDiff ok, value: ' + this.newDiff);
...
'0x'+                    heightBuffer.toString('hex')
        ...
        this.sendNewJob = function() {
            ...
            return this.messageSender('', job, 5);
        }
    }
}
...
function processShare(miner, job, blockTemplate, params, sendReply) {
	...
	console.log('nonce from miner: ' + params[0]);
...
let nonceReversed = new Buffer(nonceBuffer.toByteArray().reverse());
	console.log('nonce reversed: ' + nonceReversed.toString('hex'));

    let hashBuffer = new Buffer(resultHash, 'hex');
	console.log('hash Buffer: ' + hashBuffer.toString('hex'));
let hashBufferReversed = new Buffer(hashBuffer.toByteArray().reverse());
	console.log('hash Buffer reversed: ' + hashBufferReversed.toString('hex'));

	console.log('global.coinFuncs.getBlobFromBlockTemplate: blockTemplate.buffer: ' + blockTemplate.buffer.toString('hex') + '; extraNonceBuffer: ' + extraNonceBuffer.toString('hex') + '; nonceBuffer: ' + nonceBuffer.toString('hex'));
    let shareBuffer = global.coinFuncs.getBlobFromBlockTemplate(blockTemplate.buffer, extraNonceBuffer, nonceReversed);
		console.log('global.coinFuncs.getBlobFromBlockTemplate result: ' + shareBuffer.toString('hex'));

	let heightBuffer = Buffer.alloc(8);
                    heightBuffer.writeUInt32LE(job.height, 0);
//    let hash = global.coinFuncs.getPoWHash(hashBuffer, nonceBuffer, heightBuffer);
	let hashReversedNonce = global.coinFuncs.getPoWHash(hashBuffer, nonceReversed, heightBuffer);
//	let hashReversedBuf = global.coinFuncs.getPoWHash(hashBufferReversed, nonceBuffer, heightBuffer);
//	let hashReversedBufNonce = global.coinFuncs.getPoWHash(hashBufferReversed, nonceReversed, heightBuffer);
    let shareType = false;

     // let hashArray = hash.toByteArray().reverse();
   // let hashDiff = bignum.fromBuffer(new Buffer(hashArray));
//console.log('hash len: ' + hash.length + '; hash: ' + hash.toString('hex'));
	console.log('hash reversed nonce len: ' + hashReversedNonce.length + '; hash reversed nonce: ' + hashReversedNonce.toString('hex'));
//	console.log('hash reversed buf len: ' + hashReversedBuf.length + '; hash reversed buf: ' + hashReversedBuf.toString('hex'));
//	console.log('hash reversed buf nonce len: ' + hashReversedBufNonce.length + '; hash reversed buf nonce: ' + hashReversedBufNonce.toString('hex'));
//    let hashDiff = bignum.fromBuffer(hash);
	     let hashDiff = bignum.fromBuffer(hashReversedNonce);
	console.log('block diff ' + blockTemplate.difficulty.toString(16));
    let blockTarget = baseDiff.div(bignum(blockTemplate.difficulty));
	console.log('job diff ' + job.difficulty.toString(16));
    let jobTarget = baseDiff.div(bignum(job.difficulty));
	console.log('hashDiff: ' + hashDiff.toString(16) + '; blockTarget: ' + blockTarget.toString(16) + '; jobTarget: ' + jobTarget.toString(16));

    if (hashDiff.le(blockTarget)) {
        // Submit block to the RPC Daemon.
        // Todo: Implement within the coins/<coin>.js file.
	    console.log('submitting nonce: ' + nonceReversed.toString('hex'));
...
function handleMinerData(method, params, ip, portData, sendReply, pushMessage) {
    ...
    let difficulty, minerId, job, now, nonce_test, shareAccepted, blockTemplate;
    switch (method) {
        case 'login':
...
//		    console.log('sending message: ' + JSON.stringify(obj));
            ...
            socket.on('data', function (d) {
                dataBuffer += d;
                if (Buffer.byteLength(dataBuffer, 'utf8') > 10 * 1024 * 1024) { //10MB
                    dataBuffer = null;
                    console.warn(threadName + 'Excessive packet size ' + Buffer.byteLength(dataBuffer, 'utf8') + ' from: ' + socket.remoteAddress);
                    socket.destroy();
                    return;
                }
...
//			    console.log('new message: ' + JSON.stringify(jsonData));
