...
function newBlockTemplate(template) {
    ...
    if (activeSeed !== activeBlockTemplate.seed) {
        console.log('New seed: ', activeBlockTemplate.seed);
        activeSeed = activeBlockTemplate.seed;
        const ht = typeof(activeBlockTemplate.height);
        if (ht !== 'number') {
            console.warn('Bad height type: ' + ht);
        } else {
            activeScratchpad = global.coinFuncs.generateScratchpad(activeSeed, activeBlockTemplate.height);
            if (!Buffer.isBuffer(activeScratchpad)) {
                console.warn("Bad scratchpad type");
            } else {
                console.log("Generated new scratchpad, length: " + activeScratchpad.length);
            }
        }
    }
    for (let minerId in activeMiners) {
...
function processShare(miner, job, blockTemplate, params, sendReply) {
    ...
    if (global.config.pool.trustedMiners && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 &&
        ...
        if (typeof activeScratchpad === 'undefined') {
            console.warn('No active scratchpad');
        } else {
            hash = global.coinFuncs.hash(convertedBlob, activeScratchpad);
        }
        shareType = false;
    }
