...
function processShare(miner, job, blockTemplate, params, sendReply) {
    ...
    extraNonceBuffer.writeUInt32LE(job.extraNonce, 0);
    ...
    let hash = global.coinFuncs.getPoWHash(hashBuffer, nonceBuffer, job.height);
    let shareType = false;
    ...
    async.each(global.config.ports, function (portData) {
        ...
        function socketConn(socket) {
            ...
            socket.on('data', function (d) {
                dataBuffer += d;
                if (Buffer.byteLength(dataBuffer, 'utf8') > 10 * 1024 * 1024) { //10KB
                    dataBuffer = null;
